...
"""To sum a for loop use SUM()"""
total_connection=sum(numberOfFriends(user) for user in users)
#print total_connection
nbUsers=len(users)
avg_connections=total_connection/nbUsers
...
def data_scientist_who_like(target_interest):
    return[user_id for user_id,user_interest in interests
        if(target_interest==user_interest)]
...
def most_common_interests_with(user):
    return Counter(interest_by_user_id
        for interest in interest_by_user_id[user["id"]]
        for interest_by_user_id in user_ids_by_interest[interest]
        if interest_by_user_id != user["id"])
...
def tenure_bucket(tenure):
    if tenure <2:
        ...
        return "Between 2 years and 5 years"
    else:
